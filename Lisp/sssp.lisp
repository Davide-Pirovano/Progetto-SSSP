...
(defun new-graph (graph-id)
  (if 
   (is-graph
    graph-id)
   (format t "Errore: Un grafo con tale nome esiste gi�."
    nil)
   (setf 
    (gethash 
     graph-id 
...
(defun new-vertex (graph-id vertex-id)
  ...
  (if
   (is-vertex
    graph-id
    vertex-id)
   (format t "Errore: Il vertice � gi� presente nel grafo.."
    nil)
   (setf
    (gethash
     (list
...
(defun new-edge (graph-id vertex-1-id vertex-2-id &optional (weight 1))
  ...
  (if
   (is-edge
    ...
    vertex-1-id
    vertex-2-id)
   (format t "Errore: Tale arco � gi� presente nel grafo."
    nil)
   (if (and 
  ...
  (if (graph-vertex-neighbors graph-id source-id)
      (and
       (dijkstra
        ...
        source-id)
       nil)
    (format t "Errore: Non � possibile raggiungere alcun nodo."
    nil)))
...
;implementata perch� remove non dava i risultati aspettati
